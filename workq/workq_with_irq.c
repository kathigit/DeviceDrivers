...
//static struct delayed_work myDwq;
...
/* adding work to system work queue - static method*/

DECLARE_DELAYED_WORK(myDwq,wq_handler);
/*********************************************************************************************/
static void wq_handler(struct work_struct *work) {
	printk("Delayed WorkQueue callled\n");
}
...
static irqreturn_t gpio_irq_handler(int irq,void *dev_id){
    ...
    schedule_delayed_work(&myDwq,msecs_to_jiffies(4000));
    printk("Scheduled the work\n");
    return IRQ_HANDLED;
}
...
static int __init char_drv_init(void) {
    ...
    //INIT_DELAYED_WORK(&myDwq,wq_handler);
